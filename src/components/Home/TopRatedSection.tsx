...
export default function TopRatedSection() {
  ...
  useEffect(() => {
    ...
    async function fetchTopRated() {
      try {
        const response = await fetch(`/api/restaurants/getTopRatedRestaurants?take=6`);
    ...
    <section className="container">
      ...
      <div className="top_rated_list">
        {topRated.map((restaurant) => (
          <Link key={restaurant.id} href={`/restaurants/${restaurant.id}`} className="top_rated_card">
            <img
            ...
            <div className="top_rated_info">
              ...
              <div className="top_rated_stats">
                ...
                <div className="stat_item">
                  <LuClock size={16} />
                  <span className="stat_value">{restaurant.openHour} - {restaurant.closeHour}</span>
                </div>
              </div>
            </div>
          </Link>
